...
`vesedge_qc.json` records the exact QC configuration and input selection. `qc_summary.csv` reports extraction failures, curvature rejections, area-deviation rejections, optional radius-deviation rejections, accepted-frame counts, and accepted fractions for each checkpoint. Each trajectory also receives an area-versus-frame diagnostic plot.
