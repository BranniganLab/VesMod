...
def test_post_init_starts_with_empty_detections(extraction_config, qc_config):
    """Test that a newly constructed video has no edge-extraction results."""
    video = VesicleVideo(np.zeros((3, 200, 200)), extraction_config, qc_config)
    assert video.detections == []


def test_post_init_requires_numpy_array(extraction_config, qc_config):
    """Test that frames must be a NumPy ndarray."""
    with pytest.raises(TypeError):
        VesicleVideo([], extraction_config, qc_config)


def test_post_init_requires_3d_array(extraction_config, qc_config):
    """Test that frames must be three-dimensional."""
    with pytest.raises(IndexError):
        VesicleVideo(np.zeros((10, 10)), extraction_config, qc_config)


def test_edge_extraction_config_requires_positive_pixels_per_micron():
    """Test that pixels_per_micron must be positive."""
    with pytest.raises(ValueError):
        EdgeExtractionConfig(pixels_per_micron=0, n_angular_samples=120)
...
def test_edge_extraction_config_converts_integer_valued_sample_count():
    ...
    config = EdgeExtractionConfig(pixels_per_micron=1.0, n_angular_samples=120.0)
...
def test_edge_extraction_config_rejects_non_integer_sample_count():
    ...
    with pytest.raises(ValueError, match="integer-valued"):
        EdgeExtractionConfig(pixels_per_micron=1.0, n_angular_samples=120.5)
...
def test_edge_extraction_config_requires_positive_n_angular_samples():
    ...
    with pytest.raises(ValueError):
        EdgeExtractionConfig(pixels_per_micron=1.0, n_angular_samples=0)


def test_edge_extraction_config_allows_no_downsampling():
    """Test that None disables contour downsampling."""
    config = EdgeExtractionConfig(pixels_per_micron=1.0, n_angular_samples=None)
...
def test_post_init_rejects_too_many_samples(qc_config):
    ...
    with pytest.raises(IndexError):
        VesicleVideo(np.zeros((2, 50, 50)), extraction_config, qc_config)
...
def test_post_init_allows_no_downsampling(qc_config):
    ...
    video = VesicleVideo(np.zeros((2, 50, 50)), extraction_config, qc_config)
...
def test_compile_edge_detection_results_stores_full_contour(video):
    ...
    edge = video._compile_edge_detection_results(r_vals, vesicle_center)
    assert isinstance(edge, EdgeDetection)
    assert edge.full_contour.origin == (50.0, 60.0)
    np.testing.assert_array_equal(edge.full_contour.r, r_vals)
...
def test_compile_edge_detection_results_downsamples_analysis_contour(video):
    ...
    edge = video._compile_edge_detection_results(r_vals, (60.0, 50.0))
...
def test_compile_edge_detection_results_converts_radii_to_microns(video):
    ...
    r_vals = np.full(200, 20.0)
    edge = video._compile_edge_detection_results(r_vals, (60.0, 50.0))
    np.testing.assert_allclose(edge.radii_microns, 10.0)


def test_compile_edge_detection_results_uses_full_contour_when_not_downsampling(qc_config):
    ...
    np.testing.assert_array_equal(edge.radii_microns, np.full(200, 10.0))
...
def test_extract_edges_records_successful_detections(monkeypatch, extraction_config, qc_config):
    """Test that successful extractor results are stored as EdgeDetection objects."""
    video = VesicleVideo(np.zeros((2, 200, 200)), extraction_config, qc_config)
    ...
    assert all(isinstance(result, EdgeDetection) for result in video.detections)


def test_extract_edges_raises_when_all_extractions_fail(extraction_config, qc_config):
    ...
    assert len(video.detections) == 2
    assert all(isinstance(result, EdgeDetectionFailure) for result in video.detections)
...
def test_extract_edges_preserves_frame_order_after_failure(monkeypatch, extraction_config, qc_config):
    """Test that each extraction result retains its position in video frame order."""
    frames = np.zeros((3, 200, 200))
    frames[1] = 1.0
    video = VesicleVideo(frames, extraction_config, qc_config)
...
def test_extract_edges_rejects_inconsistent_detection_lengths(monkeypatch, qc_config):
...
def test_extract_edges_raises_when_no_frames_pass_qc(monkeypatch, extraction_config, qc_config):
...
def test_save_edge_to_npy_only_saves_accepted_detections(tmp_path, video):
    ...
    accepted_edge = video._compile_edge_detection_results(np.full(200, 20.0), (60.0, 50.0))
    rejected_edge = video._compile_edge_detection_results(np.full(200, 30.0), (60.0, 50.0))
    rejected_edge.qc.flags.add(QCFlag.CURVATURE)
    video.detections.extend([accepted_edge, EdgeDetectionFailure("failure"), rejected_edge])
    outfile = tmp_path / "edges"
    video.save_edge_to_npy(outfile)
    saved = np.load(outfile.with_suffix(".npy"))
    assert saved.shape == (1, 120)
    np.testing.assert_array_equal(saved[0], accepted_edge.radii_microns)


def test_extract_edges_replaces_previous_detections(monkeypatch, extraction_config, qc_config):
    ...
    assert all(
        second is not first
        for second, first in zip(video.detections, first_detections, strict=True)
    )
...
def test_run_trajectory_qc_calls_population_check(monkeypatch, video):
    ...
    assert observed["max_minor_fraction"] == video.qc_config.max_minor_population_fraction
    assert video.population_result is expected_result


def test_make_vesicle_gif_with_trace_requires_detections(video, tmp_path):
    """Verify traced GIFs cannot be rendered before edge extraction."""
    with pytest.raises(ValueError, match="detections"):
        video.make_vesicle_gif(tmp_path / "vesicle.gif", show_trace=True)
