...
from vesmod.VesEdge import EdgeQCConfig, SingletonDeviationQCConfig
...
def test_singleton_qc_rejects_narrow_excursion_and_registry_runs_it():
    ...
    config = EdgeQCConfig(curvature=CurvatureQCConfig(threshold=1, enabled=False), singleton=SingletonDeviationQCConfig(enabled=True))
