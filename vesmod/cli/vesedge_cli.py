...
def _add_qc_parser(subparsers) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "QC; must be paired with --max-localized-deviation-support-samples."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Maximum contiguous angular support for the secondary localized-"
            "deviation threshold."
        ),
    )
    ...
    return EdgeQCConfig(
        ...
        baseline=LocalizedDeviationQCConfig(
            ...
            support_residual_fraction=getattr(
                args, "localized_deviation_support_residual_fraction", None
            ),
            max_support_samples=getattr(
                args, "max_localized_deviation_support_samples", None
            ),
        ),
...
def process_qc_file(
    ...
    try:
        ...
        if qc_config.internal_vesicle.enabled:
            ...
            source_path = resolve_source_path(edges.source_path, path)
            frames = open_checkpoint_frames(source_path)
        if frames is None:
